  ...
  handleSubmit(event) {
    ...
    // }).then(res => {
      // if (res.data.result) {
        ...
        //   this.handleJoin()
        // })
        axios.post(`${config.BE_Domain}/session/create-new-session`, {
          "size": this.state.size,
          "topicGameId": this.state.tag
        }).then(res => {
          // console.log(res)
          console.log(res)
          this.setState({roomPIN: res.data.roomPIN}, function() {
    ...
    return (
      <ResponsiveContainer>
        <Form onSubmit={this.handleSubmit} className="formChoose">
            ...
            <select class="form-control" id="exampleFormControlSelect1"  onChange={(e) => { this.setState({tag: e.target.value})}}>
              {/* <option></option> */}
              {this.state.tagOptions.map(tagOption => 
                (<option value={tagOption._id}>{tagOption.name}</option>)
              )}
            </select>
              <div className="redText">{this.state.errRoom}</div>
            <Input
